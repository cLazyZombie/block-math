...
만 4세 아이가 알록달록한 하나의 블록 덩어리를 보고 숫자를 드래그해서 맞히는 정적 웹 게임입니다. 10은 세로 10칸 기둥이고, 두 자리 이상은 그 기둥들이 붙어서 한 몸체처럼 보입니다.
...
4. 숫자 타일은 누르기만 하면 소리가 나지 않고, 빈칸에 넣을 때 자리값에 맞춰 읽어 줍니다.
5. 숫자 타일은 사라지지 않아서 같은 숫자를 여러 번 넣을 수 있습니다.
6. 맞추면 1초 동안 완성된 답을 보여 준 뒤 `참 잘했어요!`와 함께 숫자를 한국어로 읽어 줍니다.
